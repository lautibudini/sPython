...
with st.container():
    with st.form(key='form1'):
        ...
        text_gender = st.text_input('Ingrese su género')
        
        # Verificar si el formulario ha sido enviado
        if st.form_submit_button('Enviar'): 
            # Verificar si todos los campos están completos
            aux = functions_streamlit.verify(text_name, text_real_name, text_mail, text_gender)
            if aux:
                # Verificar si el correo electrónico es válido
                mail = functions_streamlit.verifymail(text_mail)
                if mail:
                    # Crear datos del nuevo usuario
                    new_user_data = functions_streamlit.new_user(text_name, text_real_name, text_mail, text_gender)
                    # Agregar el usuario a los datos existentes
                    functions_streamlit.add_user(new_user_data, path_user_data)
